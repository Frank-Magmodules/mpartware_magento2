                ...
                source.templates.footer = function (query, content) {
                    ...
                    if (content.nbHits > 0) {
                        ...
                        content.hits.forEach(hit => {
                            ...
                            test += 1;
                            let str = hit._highlightResult.name.value;
                            str = str.substring(
                                str.lastIndexOf("<em>") + 4,
                                str.lastIndexOf("</em>")
                            );
                            elasticHits.push(' '+str+' ');
                        });

                        elasticHits = elasticHits.filter(function (element, index, self) {
                    ...
                    if (query.query.length >= 3) {
                        let productHits = (content.nbHits - content.nbPages);
                        var allUrl = algoliaConfig.baseUrl + '/catalogsearch/result/?q=' + encodeURIComponent(elasticHits);
