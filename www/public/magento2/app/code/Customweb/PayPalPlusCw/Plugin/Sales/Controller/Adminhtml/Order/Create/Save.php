...
namespace Customweb\PayPalPlusCw\Plugin\Sales\Controller\Adminhtml\Order\Create;
...
class Save
{
	...
	public function aroundExecute(\Magento\Sales\Controller\Adminhtml\Order\Create\Save $subject, \Closure $proceed)
	{
		...
		$this->_coreRegistry->register('paypalpluscwcheckout_moto', true);
		$result = $proceed();
		if ($this->_messageManager->getMessages()->getLastAddedMessage() != null
			...
			$order = $this->_coreRegistry->registry('paypalpluscw_checkout_last_order');
			if ($order instanceof \Magento\Sales\Model\Order) {
				if ($order->getPayment()->getMethodInstance() instanceof \Customweb\PayPalPlusCw\Model\Payment\Method\AbstractMethod) {
					...
					return $this->_redirectResultFactory->create()->setPath('paypalpluscw/checkout/payment', ['order_id' => $order->getId(), 'send_confirmation' => $sendConfirmation]);
