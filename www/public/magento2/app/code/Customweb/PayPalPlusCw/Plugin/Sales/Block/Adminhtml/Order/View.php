...
namespace Customweb\PayPalPlusCw\Plugin\Sales\Block\Adminhtml\Order;
...
class View
{
	...
	public function aroundGetReviewPaymentUrl(\Magento\Sales\Block\Adminhtml\Order\View $subject, \Closure $proceed, $action)
    {
    	if ($subject->getOrder() instanceof \Magento\Sales\Model\Order
	    		...
	    		&& $subject->getOrder()->getPayment()->getMethodInstance() instanceof \Customweb\PayPalPlusCw\Model\Payment\Method\AbstractMethod) {
	    	$transaction = $this->_transactionFactory->create()->loadByOrderId($subject->getOrderId());
	    	if ($transaction->getId()) {
	    		return $subject->getUrl('paypalpluscw/transaction/reviewPayment', ['action' => $action, 'transaction_id' => $transaction->getId()]);
	    	}
    	}
    ...
    {
		$this->_coreRegistry->register('paypalpluscw_order_view', true, true);
    }
}