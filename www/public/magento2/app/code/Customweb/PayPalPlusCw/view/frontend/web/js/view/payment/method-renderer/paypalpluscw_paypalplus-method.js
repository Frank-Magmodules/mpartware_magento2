...
define([
	...
	'Customweb_PayPalPlusCw/js/checkout',
	'Customweb_PayPalPlusCw/js/authorizationMethod',
	'Customweb_PayPalPlusCw/js/alias'
], function(
	$,
	Component,
	mageTemplate,
	...
	return Component.extend({
		defaults: {
			template: 'Customweb_PayPalPlusCw/payment/paypalpluscw_paypalplus',
			fieldErrorTemplate: '<div for="<%- id %>" generated="true" class="mage-error" id="<%- id %>-error"><%- message %></div>',
			authorizationUrl: url.build('paypalpluscw/checkout/authorize/'),
			restoreCartUrl: url.build('paypalpluscw/checkout/restoreCart/')
		},
		...
		getForm: function() {
			...
			if (!customer.isLoggedIn()) {
				serviceUrl = urlBuilder.createUrl('/guest-carts/:cartId/paypalpluscw/checkout/payment-form/:paymentMethod', {
					cartId: quote.getQuoteId(),
					paymentMethod: this.item.method
				});
			} else {
				serviceUrl = urlBuilder.createUrl('/carts/mine/paypalpluscw/checkout/payment-form/:paymentMethod', {
					paymentMethod: this.item.method
				});
			}
			storage.get(
