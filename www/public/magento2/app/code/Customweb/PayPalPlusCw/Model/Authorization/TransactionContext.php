	...
	\Customweb_Payment_Authorization_IUpdateTransactionContext
{
	...
	/**
	 * @var \Customweb\PayPalPlusCw\Model\Authorization\CustomerContextFactory
	 */
	...
	/**
	 * @var \Customweb\PayPalPlusCw\Model\Authorization\OrderContext
	 */
	 ...
	 * @param \Customweb\PayPalPlusCw\Model\Authorization\CustomerContextFactory $customerContextFactory
	 * @param \Customweb\PayPalPlusCw\Model\Authorization\Transaction $transaction
	 * @param \Customweb\PayPalPlusCw\Model\Authorization\OrderContext $orderContext
	 * @param int $orderId
	...
	public function __construct(
			...
			\Customweb\PayPalPlusCw\Model\Authorization\CustomerContextFactory $customerContextFactory,
			\Customweb\PayPalPlusCw\Model\Authorization\Transaction $transaction,
			\Customweb\PayPalPlusCw\Model\Authorization\OrderContext $orderContext,
			$orderId,
		...
		$this->backendSuccessUrl = $backendHelper->getUrl('paypalpluscw/checkout/success');
		$this->backendFailureUrl = $backendHelper->getUrl('paypalpluscw/checkout/failure');
	}
	...
	{
		$this->_transactionFactory = \Magento\Framework\App\ObjectManager::getInstance()->get('Customweb\PayPalPlusCw\Model\Authorization\TransactionFactory');
		$this->_customerContextFactory = \Magento\Framework\App\ObjectManager::getInstance()->get('Customweb\PayPalPlusCw\Model\Authorization\CustomerContextFactory');
	}
	...
	{
		return $this->getTransaction()->getStore()->getUrl('paypalpluscw/checkout/success', ['_secure' => true]);
	}

	public function getFailedUrl()
	{
		return $this->getTransaction()->getStore()->getUrl('paypalpluscw/checkout/failure', ['_secure' => true]);
	}

	public function getIframeBreakOutUrl()
	{
		return $this->getTransaction()->getStore()->getUrl('paypalpluscw/checkout/breakout', ['_secure' => true]);
	}
	...
	{
		if (!($this->cachedTransaction instanceof \Customweb\PayPalPlusCw\Model\Authorization\Transaction)) {
