...
class ConfigProvider implements \Magento\Checkout\Model\ConfigProviderInterface
{
    ...
    public function __construct(
        ...
        foreach (array_keys($paymentHelper->getPaymentMethods()) as $code) {
        	if (strpos($code, 'paypalpluscw_') === 0) {
    ...
    protected function getFailureMessage(AbstractMethod $method)
    {
    	if ($this->_checkoutSession->getQuote()->getPayment()->getMethod() == $method->getCode()) {
	    	$failureMessage = $this->_checkoutSession->getPayPalPlusCwFailureMessage();
	    	if (!empty($failureMessage)) {
	    		$this->_checkoutSession->unsPayPalPlusCwFailureMessage();
    ...
    protected function getImageUrl(AbstractMethod $method)
    {
    	try {
    		...
    		return $this->_assetRepo->getUrlWithParams('Customweb_PayPalPlusCw/images/payment/method/' . $method->getPaymentMethodName() . '.png', $params);
    ...
    {
    	return new \Customweb\PayPalPlusCw\Model\Renderer\CheckoutForm($method->getCode());
    }
